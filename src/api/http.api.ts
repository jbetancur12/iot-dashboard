...
import { persistRefreshToken, persistToken, readRefreshToken, readToken } from '@app/services/localStorage.service';
...
httpApi.interceptors.request.use((config) => {
  config.headers = { Authorization: `Bearer ${readToken()}`, 'x-access-token': readToken() } as CommonHeaderProperties;
  return config;
});
