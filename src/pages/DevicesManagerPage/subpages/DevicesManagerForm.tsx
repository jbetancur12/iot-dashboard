...
async function fetchUserList(username: string): Promise<UserValue[]> {
  ...
  return fetch(
    `${process.env.REACT_APP_BASE_URL}api/users/?email=${username}`,
    {
      method: 'GET',
      headers: {
...
const DevicesManagerForm: React.FC = () => {
  ...
  const handleSubmit = (values: DeviceFormData) => {
    ...
    setLoading(true)
    return isAddMode
      ? createDevice(valuesToSend)
      : updateDevice(id, valuesToSend)
  }
  ...
  const getUser = async (id: string | undefined) => {
    ...
    // @ts-ignore
    form.setFields([
      { name: 'user', value: { label: user.email, value: user.id } }
    ])
  }
