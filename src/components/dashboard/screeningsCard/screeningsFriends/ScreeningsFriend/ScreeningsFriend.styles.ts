...
export const ScreeningsRow = styled(Row)<ScreeningsRowProps>`
  ...
  .ant-typography {
    font-weight: ${(props) =>
      props.$isActive
        ? props.theme.commonFontWeight.bold
...
export const Avatar = styled(AntAvatar)<Importance>`
  ...
  ${(props) =>
    (props.$isPrimary &&
      `border: 2px solid ${props.theme.colors.main.primary}`) ||
...
export const Name = styled(Typography.Text)<Importance>`
  ...
  color: ${(props) =>
    (props.$isPrimary && props.theme.colors.main.primary) ||
    (props.$isSecondary && props.theme.colors.main.error)};
